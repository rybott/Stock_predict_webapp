...
# Function to read the Excel file path from config.txt
def get_excel_path():
    try:
        ...
        messagebox.showerror("Error", "Configuration file 'config.txt' not found.")
        return None

# Function to find the last row where a value appears in a specific column
def find_last_row(column_name, search_value, df):
...
def process_input():
    ...
    if not excel_path or not word_path or not save_path or not search_value:
    ...
    try:
        ...
        messagebox.showerror("Error", f"Failed to load Excel file. Please check the file path.\nError: {str(e)}")
        return

    column_name = 'Company Name'

    # Find the last row where the value appears
    last_row_data = find_last_row(column_name, search_value, df)
    ...
    if last_row_data is not None:
        ...
        messagebox.showwarning("Not Found", f"The value '{search_value}' was not found in the Form Database.")
...
tk.Label(root, text="Enter the individuals name:").pack(pady=10)
